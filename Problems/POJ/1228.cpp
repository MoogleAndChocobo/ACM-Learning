...
int gcd(int a, int b);bool containOn(const Polygon_convex a, Point b);
...
struct Polygon
{
	...
	bool isConvex()
	{
		...
		for(; cnt <= n - 1; ++cnt)
		{
			int cur = ccw(a[cnt], a[cnt + 1], a[cnt + 2]);
			if(cur == 1 || cur == 0)
			{
				now = cur;
				break;
...
bool containOn(const Polygon_convex a, Point b)
{
	int n = a.P.size();
	#define next(i) ((i+1)%n)
	int sign = 0;
...
{   
	...
	rep(kase, 1, T)
	{
		...
		rep(i, 0, n - 3)
		{
			int now = ccw(vec[i], vec[i + 1], vec[i + 2]);
			if(now != 2 && now != 3)
			{
				check = true;
				break;
		...
		rep(i, 0, len - 1)
		{
			...
			rep(j, 0, n - 1)
			{
				if(PointOnSegment(vec[j], ans[i], ans[i + 1]))
					cnt++;
			}
			if(cnt < 3)
			{
