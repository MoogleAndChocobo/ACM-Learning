...
double calc(int n)
{
	double sum = 0;
	rep(i, 1, n - 2)
		sum += triangle(rec[0], rec[i], rec[i + 1]);
	return sum;
}
...
{   
	...
	while(~sfi(n) && n >= 3)
	{
		Point x;
		double r;
		scanf("%lf%lf%lf", &r, &x.x, &x.y);
		polygon.n = n;
		rep(i, 0, n - 1)
			scanf("%lf%lf", &rec[i].x, &rec[i].y);
		rep(i, 0, n - 1)
			polygon.a[i].x = rec[i].x, polygon.a[i].y = rec[i].y;
		
		rec[n] = rec[0], rec[n + 1] = rec[1];
		int now = 1;
		int cnt = 0;
		for(; cnt <= n - 1; ++cnt)
		{
			int cur = ccw(rec[cnt], rec[cnt + 1], rec[cnt + 2]);
			if(cur == 1 || cur == 0)
			{
				now = cur;
				break;
		...
		{
			rep(i, 0, n - 1)
				polygon.a[i].x = rec[n - 1 - i].x, polygon.a[i].y = rec[n - 1 - i].y;
		}


		if(polygon.Point_In(x) != 1)
		...
		else
		{
			...
			rep(i, 0, n - 2)
			{
				double now = dis_point_segment(x, Line(rec[i], rec[i+1]));
				if(now < r)
				{
					f = false;
					break;
